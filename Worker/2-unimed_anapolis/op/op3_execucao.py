"""
Op 3 — Execução de Guias SP/SADT (Unimed Anápolis)
Fluxo completo de faturamento no portal SGUCard baseado em modelo_execucao_anapolis.py.
Pré-requisito: Job de Captura pai com status 'success' (enforced pelo dispatcher via depending_id).

Parâmetros esperados em job_data['params']:
  - numero_guia       : str  — número da guia a ser executada
  - agendamento_id    : int  — para atualização de execucao_status no banco
  - nome_profissional : str  — nome do profissional (para busca na lupa)
  - conselho          : str  — código do conselho (ex: "CRP", "CREFONO", "CREFITO")
  - data_hora         : str  — "dd/mm/aaaa hh:mm" (data+hora da sessão)
  - cod_procedimento_fat : str — código de faturamento (ex: "2250005170")

Fases:
  FASE 1: Navegação → SADTs em Aberto (reusa _navigate_to_sadts de op2)
  FASE 2: Filtro s_nr_guia (reusa _apply_guia_filter de op2)
  FASE 3: Abre guia → Tipo Atendimento "03 - Outras Terapias" → Regime "01 - Ambulatorial"
           → preenche dt_serie_1 com data_hora → clica Gravar
  FASE 4: Aguarda listagem → clica ícone de vínculo → clica Nova Participação
  FASE 5: Clica lupa prestador → janela abre → preenche nome → seleciona conselho
           → seleciona Prestador Externo → Filtrar → clica 1º resultado → janela fecha
  FASE 6: Clica Cadastrar → grau "12 - Clínico" → seleciona item cod_procedimento_fat → Gravar
  FASE 7: Clica "Dados da Guia SP/SADT" → Finalizar Parcial → atualiza banco
"""
import time
import json
...
# Reutiliza helpers DRY de op2_captura
from op.op2_captura import _navigate_to_sadts, _apply_guia_filter, _robust_click


# ── Mapeamento de Conselho ──────────────────────────────────────────────────
# O select name="s_tp_conselho" no portal usa a label completa.
# Mapeamos a sigla do CorpoClinico para o prefixo da option no select.
...
def _select_conselho(driver, conselho_code, log):
    """
    Seleciona o conselho no select name="s_tp_conselho" usando match de prefixo.
    conselho_code: valor vindo do CorpoClinico.conselho (ex: "CRP", "CREFONO")
    """
    # Normaliza: remove sufixo como "-GO" para match no mapa
    ...
    try:
        ...
        # Fallback: match parcial no texto
        for option in sel.options:
...
def _select_option_by_partial_text(driver, element_id, partial_text, log, by=By.ID):
    """
    Seleciona uma option em um <select> por texto parcial.
    Útil para NR_SEQ_ITEM_1 (cod_procedimento_fat).
    """
    try:
        select_el = WebDriverWait(driver, 10).until(
...
def execute(scraper, job_data):
    """
    Executa a Rotina 3 (Execução/Faturamento SP/SADT) para Unimed Anápolis.
    A guia já deve estar capturada (depending_id enforced pelo dispatcher).
    """
    ...
    log(
        f"Op3 Execução: Guia={numero_guia} | Profissional='{nome_profissional}' "
    ...
    if not guia_encontrada:
        ...
        # Erro específico solicitado pelo usuário
        raise Exception(f"PermanentError: Guia {numero_guia} não capturada ou não disponível no portal.")

    # ── FASE 3: Abre guia → Tipo/Regime/DataHora → Gravar ─────────────────────
    log(f"FASE 3: Abrindo detalhes da guia {numero_guia}...")

    # Clica no link da guia (coluna 3 da linha trRow_2)
    guia_link = WebDriverWait(driver, 15).until(
        EC.element_to_be_clickable((By.XPATH, '//tr[@id="trRow_2"]/td[3]/a'))
    )
    _robust_click(driver, guia_link, "guia_link (trRow_2/td[3]/a)", log)
    _wait_page_stable(driver)
    time.sleep(2)

    # Seleciona Tipo de Atendimento: "03 - Outras Terapias"
    try:
    ...
    # Seleciona Regime de Atendimento: "01 - Ambulatorial"
    try:
    ...
    # Preenche dt_serie_1 com data_hora da sessão
    if data_hora:
        try:
    ...
    # Clica Gravar
    log("FASE 3: Clicando em Gravar...")
    original_handles = driver.window_handles
    try:
        btn_gravar = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.ID, "Button_Gravar"))
        )
        _robust_click(driver, btn_gravar, "Button_Gravar", log)
    except Exception as e:
        log(f"FASE 3: Erro ao clicar Gravar: {e}", level="ERROR")
        raise

    # Fechar popup que pode abrir após Gravar
    time.sleep(2)
    close_alert_if_present(driver)
    new_handles = set(driver.window_handles) - set(original_handles)
    if new_handles:
        for h in new_handles:
            driver.switch_to.window(h)
            log(f"Fechando popup pós-Gravar: {driver.title}")
            driver.close()
    ...
    # ── FASE 4: Vínculo profissional — ícone + Nova Participação ──────────────
    log("FASE 4: Clicando ícone de vínculo do profissional...")
    try:
        # Ícone de vínculo: 1ª imagem no grid-menu (conforme modelo_execucao_anapolis.py step 20)
        vinculo_icon = WebDriverWait(driver, 15).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, ".grid-menu:nth-child(1) img"))
        )
        _robust_click(driver, vinculo_icon, "vinculo_icon (.grid-menu:nth-child(1) img)", log)
    except Exception as e:
        log(f"FASE 4: Ícone de vínculo não encontrado: {e}", level="ERROR")
        raise

    time.sleep(2)

    # Clica em Nova Participação
    try:
    ...
    # Seleciona tipo busca: "Prestador Externo"
    try:
    ...
    # Clicar no 1º resultado listado
    try:
        primeiro_resultado = None
        
        # 1. Tenta por atributo data-nm-prest (se o portal fornecer)
        try:
            ...
            # 2. Tenta pelo XPath preciso fornecido pelo usuário
        ...
        # Fecha a janela de busca se ainda estiver aberta
        try:
            if len(driver.window_handles) > 1:
                # Se estivermos na janela secundária (popup), fecha ela
    ...
    # Aguarda janela fechar e retorna para janela principal
    time.sleep(2)
    ...
    # Seleciona Grau de Participação: "12 - Clínico"
    try:
    ...
    # Seleciona Item (NR_SEQ_ITEM_1) pelo código de faturamento
    if cod_proc_fat:
    ...
    # Clica Finalizar Parcial e Confirmar
    ...
    try:
        ...
        # Novo passo: Janela de confirmação
        log("FASE 7: Aguardando abertura do popup de confirmação final...")
        
        # Lógica igual à Fase 5 (Busca de Prestador)
        ...
        if popup_confirmacao:
            ...
            # Agora busca o btn_confirmar *dentro* da nova janela!
            ...
            # Aguarda e retorna o controle para a janela principal
            time.sleep(2)
            ...
            driver.switch_to.window(handles_before_parcial[0])
             ...
             # Fallback caso não seja janela e sim Modal via DOM puro
