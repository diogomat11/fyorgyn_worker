...
5. **Retorno das Guias e Persistência Externa (Desacoplamento do Banco de Dados):**
   - As guias coletadas ao longo de todas as páginas são agregadas em uma lista de resultados.
   - A persistência local direta no banco de dados do worker foi desabilitada para manter o total desacoplamento e isolamento do banco público de produção.
   - O `Worker` retorna a lista de guias raspadas no JSON de resultado do Job.
   - O `backend_worker` devolve essa matriz no payload do webhook cadastrado.
   - O Hub Backend, ao receber a notificação via webhook, processa os dados, valida a propriedade e a associação de `user_id` e carteirinhas de forma isolada e multitenant, gravando-os em seu próprio banco.
